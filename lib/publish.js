...
function publishFromPackage (arg, opts) {
  return cacache.tmp.withTmp(opts.tmp, {tmpPrefix: 'fromPackage'}, tmp => {
    ...
    return pacote.tarball.toFile(arg, target, pacOpts)
      .then(() => pacote.extract(arg, extracted, pacOpts))
  ...
  if (!opts.dryRun) {
    return readFileAsync(cached).then(tarball => {
      return otplease(opts, opts => {
        return libpub.publish(pkg, tarball, opts)
      }).catch(err => {
        if (
          ...
          return otplease(opts, opts => libpub.unpublish(
